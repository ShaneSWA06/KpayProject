  ...
  if (url.pathname === "/api/transactions") {
    ...
    if (req.method === "POST") {
      ...
      const duplicate = allowDuplicate
        ? null
        : await findSameDayDuplicateTransaction({ customerName, phoneNumber, amount, type, referenceStamp: createdAt });
...
async function findSameDayDuplicateTransaction({ customerName, phoneNumber, amount, type, referenceStamp }) {
  ...
  const result = await pool.query(
    `SELECT
     ...
     ORDER BY created_at DESC
     LIMIT 5`,
    [normalizedCustomerName, amount, normalizedPhoneNumber, referenceDate, type]
  );
...
async function findRecentDuplicateTransaction({ customerName, phoneNumber, amount, type, referenceStamp, createdById, windowMinutes }) {
  const normalizedCustomerName = normalizeText(customerName);
  const normalizedPhoneNumber = normalizePhoneNumber(phoneNumber);
  const referenceDate = getStampDate(referenceStamp);
  ...
  const result = await pool.query(
    `SELECT
     ...
     ORDER BY created_at DESC
     LIMIT 10`,
    [normalizedCustomerName, amount, normalizedPhoneNumber, createdById, referenceDate, type]
  );
